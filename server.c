...
//Hold the chatDataVars and the new client's socket fd.
typedef struct {
    chatDataVars *data;
    int clientSocketFd;
} clientHandlerVars;
...
int main(int argc, char *argv[])
{
    ...
    long port = 9999;//default port num
    int socketFd;
...
//Handle the new client thread and message consumer thread
void startChat(int socketFd)
{
    chatDataVars data;
    data.numClients = 0;
    ...
    {
        fprintf(stderr, "Connection handler successed\n");
    }
    ...
    {
        fprintf(stderr, "Message handler successed\n");
    }
...
//Waits for the queue and broadcasts to clients
void *messageHandler(void *data)
{
    chatDataVars *chatData = (chatDataVars *)data;
